...
THREEDAYS        = TODAY + timedelta(seconds=5)
...
def viewBuild(name):
    ...
    for version, url, gui, kodi, themefile, icon, fanart, preview, adult, info, description in match:
        ...
        addFile('[COLOR white]INSTALAR O REPARAR[/COLOR]'   , 'install', name, 'fresh'  , description=description, fanart=fanart, icon=icon, themeit=THEME1)
        addFile('[COLOR yellow]ACTUALIZAR[/COLOR]', 'install', name, 'normal' , description=description, fanart=fanart, icon=icon, themeit=THEME1)
...
def maintMenu(view=None):
	...
	addDir ('[I]<< Volver al Menu Principal WizardK[/I]', icon=ICONMAINT, themeit=THEME2)	    
